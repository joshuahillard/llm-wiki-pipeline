...
|--------|---------|---------|
| **Tests passing** | 0 | 352/352 (59 parser + 225 validator + 76 integration + 32 promote-local; `--stage all` exits 0) |
...
| TD-002 | Local-git promotion implemented (Phase 1.8 / part 1): `Invoke-LocalGitPromotion` creates an isolated git worktree under `%TEMP%\llm-wiki-promote-<guid>`, copies `provisional/` -> `verified/`, commits on `auto/<source>/<8-hex>`, and rolls back via `git worktree remove --force` plus branch deletion on failure. JSONL fault emission (`PROMOTION_LOCAL_GIT_FAILED`) on rollback. User's working tree never modified. Earlier work (Phase 1.5): env-driven Gitea client, conservative declined-PR reconciliation, preflight startup cleanup, branch-existence check (fail-closed). OQ-2 resolved (Phase 1.5) — branch protection confirmed on external Gitea instance (private, not in repo). Remaining gaps (TD-002 part 2 / Engineering Prompt 03b): live git push of the worktree branch, `New-GiteaPullRequest` invocation + `Write-PendingPrEntry`, worktree teardown after push, full tree-SHA equivalence (P0-8 — requires git/trees API or local fetch), F7 allowlist inversion in integration stage. The Phase 1.7 hard throw at line 783 has been replaced; Promote now throws downstream of local-git with `promotion_gated_pending_remote_wiring` until 03b lands. | Medium | Phase 0.6 | **Narrowed** (part 1 closed Phase 1.8) |
...
*Last updated: April 26, 2026 (Phase 1.8 — Local Git Promotion / TD-002 part 1)*

...
| tree-match (Phase 2.0) | `tree_match` | Orphan-branch detected (mock: branch exists, no open PR), bare-repo fixture pre-populated by warmup with an equivalent tree. Production `Test-RemoteTreeEquivalence` runs against a real `file://` remote via `git fetch + rev-parse`. Verifies recovery branch: `tree_sha_check` JSONL event with `equivalent=true`, `outcome=equivalent`, `trees_match=true`, `parents_match=true`; push is SKIPPED (no `promotion_push_completed` event); PR is created via the mock; `promotion_completed.pushed_to_remote=false`, `promotion_completed.tree_sha_check=equivalent`. Closes the P0-8 recovery code path. |
| tree-mismatch (Phase 2.0) | `tree_mismatch` | Orphan-branch detected, bare-repo fixture's branch was AMENDED in warmup with a divergence marker so its tree SHA differs from what the test phase will produce locally. Verifies fail-closed: `tree_sha_check` event with `equivalent=false`, `outcome=not_equivalent_failed_closed`, `trees_match=false`, `parents_match=true` (amend keeps parent stable); script throws "non-equivalent tree state" + "Fail-closed per P0-8"; worktree + local branch rolled back; no `promotion_completed`, no `promotion_push_completed`. The throw bypasses `Write-PromotionFault` deliberately, so 0 `operational_fault` events fire — the assertion is the canary if a future refactor adds one. |

The two tree-equivalence paths use a real `file://` bare-repo fixture (built per-test by `_setup_bare_repo_for_tree_path`) so that `Test-RemoteTreeEquivalence`'s `git fetch` + `rev-parse {sha}^{tree}` runs against controlled tree state, not against a stub. The fixture itself is built by running Promote-ToVerified.ps1 once in `LLM_WIKI_GITEA_MOCK_MODE=local_only` (the "warmup") to produce a worktree + branch, then pushing that branch (or an amended version of it, for tree-mismatch) to the bare repo. The warmup worktree and its local branch ref are torn down before the test phase runs so `Invoke-LocalGitPromotion` can recreate the branch fresh.

**Limitations of the bare-repo fixture (for follow-up phases):**
- **Token-leak sweep is no-op in tests.** `Test-RemoteTreeEquivalence`'s production token-leak sweep on `.git/config` does nothing useful here — the `file://` URL carries no token, so there is nothing to leak. The sweep itself is exercised by the live throwaway-Gitea smoke test (Phase 1.9 ledger entry), not by these stages.
- **PowerShell-Python text coupling.** Two assertions check substrings of the script's `throw` message ("non-equivalent tree state", "Fail-closed per P0-8"). PowerShell's exception formatter ANSI-decorates and word-wraps these messages, so `_normalize_ps_text` in `run_harness.py` strips the ANSI escapes and `     | ` line-prefix wraps before searching. A formatting refactor of either the throw text or the PowerShell exception decorator could regress these assertions; `_normalize_ps_text` is the integration point.
- **Live orphan-recovery scenario not automated.** The fixture covers the tree-match (recovery succeeds) and tree-mismatch (fail-closed) outcomes, but does not exercise the full lifecycle of an orphan from a real prior interrupted run. That live scenario remains a manual one-off against a throwaway Gitea (Phase 2.0 follow-up).

Implemented as the `promote-full` stage in `run_harness.py`. 55 assertions across six paths; runs on `python run_harness.py --stage all` and `--stage promote-full`. Requires pwsh.

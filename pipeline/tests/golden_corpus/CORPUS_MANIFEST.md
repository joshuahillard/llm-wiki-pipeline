...
| tree-mismatch (Phase 2.0) | `tree_mismatch` | Orphan-branch detected, bare-repo fixture's branch was AMENDED in warmup with a divergence marker so its tree SHA differs from what the test phase will produce locally. Verifies fail-closed via structured signals: `tree_sha_check` event with `equivalent=false`, `outcome=not_equivalent_failed_closed`, `trees_match=false`, `parents_match=true` (amend keeps parent stable); exit code non-zero; no `promotion_completed`, no `promotion_push_completed`. The throw bypasses `Write-PromotionFault` deliberately, so 0 `operational_fault` events fire — the absence-of-fault assertion is the canary if a future refactor adds one. |
...
Implemented as the `promote-full` stage in `run_harness.py`. 53 assertions across six paths; runs on `python run_harness.py --stage all` and `--stage promote-full`. Requires pwsh.
